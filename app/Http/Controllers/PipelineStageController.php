<?php

namespace App\Http\Controllers;

use App\Models\PipelineStage;
use Illuminate\Http\Request;

class PipelineStageController extends Controller
{
    /**
     * Display a listing of the resource.
     */
    public function index()
    {
        $stages = PipelineStage::orderBy('order', direction: "asc")->get();

        return view('pipeline-stages.index', compact('stages'));
    }

    /**
     * Store a newly created resource in storage.
     */
    public function store(Request $request)
    {
        $request->validate([
            'name' => 'required|string|max:255',
            'order' => 'required|integer|min:1',
        ]);

        PipelineStage::create($request->only('name', 'order'));

        return redirect()
            ->route('pipeline-stages.index')
            ->with('success', '阶段已创建');
    }

    /**
     * Show the form for creating a new resource.
     */
    public function create()
    {
        return view('pipeline-stages.create');
    }

    /**
     * Display the specified resource.
     */
    public function show(PipelineStage $pipelineStage)
    {
        return view('pipeline-stages.show', compact('pipelineStage'));
    }

    /**
     * Show the form for editing the specified resource.
     */
    public function edit(PipelineStage $pipelineStage)
    {
        return view('pipeline-stages.edit', compact('pipelineStage'));

    }

    /**
     * Update the specified resource in storage.
     */
    public function update(Request $request, PipelineStage $pipelineStage)
    {
        $request->validate([
            'name' => 'required|string|max:255',
            'order' => 'required|integer|min:1',
            'color' => 'nullable|string|max:20',
        ]);

        $pipelineStage->update($request->only('name', 'order', 'color'));

        return redirect()
            ->route('pipeline-stages.index')
            ->with('success', '阶段已更新');
    }

    /**
     * Remove the specified resource from storage.
     */
    public function destroy(PipelineStage $pipelineStage)
    {
        $pipelineStage->delete();

        return redirect()
            ->route('pipeline-stages.index')
            ->with('success', '阶段已删除');
    }
}
